<template>
    <div class="input-wr of" v-clickoutside="handleClose">
        <div class="of-input">
            <input type="text"
                   v-model="inputText"
                   :placeholder="$t('message.service_find_example')"
            ...
            <div class="of-drop_list" v-show="inputText.length > 2 && fp.length">
                <el-scrollbar :noresize="false"
                    ...
                    <ul>
                        <li v-for="(item, index) in fp" class="of-drop_item" :tabindex="index + 1" @click="makePush(item)">
                            <div class="of-drop_text" v-html="getText(item.name_ru)"></div>
                            <div class="of-drop_price"><span v-text="'~' + item.price"></span> {{ $t('message.text_lei') }}</div>
                        </li>
                    </ul>
                </el-scrollbar>
            </div>
...
//    import Clickoutside from 'element-ui/src/utils/clickoutside';
    ...
    export default {
        ...
        methods:{
            ...
            makePush(item){
                this.selecteditem = '';
                this.selecteditem = item.id+','+item.id_unic;
