<template>
    <section class="order-data_in" v-if="progress.loginForm">
        <h2 class="order-body_title tel">{{ $t('message.phones_for_communication') }}</h2>
        <section class="order-data_enter">
            <transition name="fade" mode="out-in" tag="div" key="userPhones">
            <div class="enter-wrapper st2">
                <div class="enter-reg">
                    <div class="enter-reg_form user-phones" v-if="userPhones.length > 0">
                        <div class="phones-list">
                            <div class="phones-list__item"
                                 :class="{'selected': phone.phone === userPhoneNumber}"
                                 v-for="phone in userPhones">
                                <el-radio v-model="userPhoneNumber"
                        ...
                        <div class="enter-reg_form phones" v-if="newPhoneNumberForm">
                            <div class="input-wr tel">
                                ...
                                <span :class="{ error: newPhoneNumberErrors.phone.hasErrors }"
                                      ...
                                      v-cloak></span>
                                <span :class="{ error: userPhoneErrors.hasError }"
                                      v-if="userPhoneErrors.hasError && userPhones.length === 0 && newPhoneNumber.length === 0"
                                      v-text="userPhoneErrors.errorText"
                                      v-cloak></span>
                            </div>
                                    ...
                                    <div class="input-wr tel">
                                        ...
                                        <div class="control-wrapper">
                                            <input type="text"
                                                   ...
                                                   novalidate
                                                   v-on:keypress="keyValidator($event,newPhoneCode,false)"
...
<script>
    export default {
        ...
        methods:{
            ...
            setupPhones(){
                console.log('setupPhones()');
                if(this.progress.loginForm){
                    console.log('setupPhones()');
                    ...
                    if(this.userPhones.length !== 0) {
                        this.userPhoneNumber = this.userPhones[0].phone;
                        this.userPhones = this.initialUserPhones;
                    }
                }
            },
            showNewPhoneNumberForm(){
                //console.log('showNewPhoneNumberForm: ', this.newPhoneNumberForm);
